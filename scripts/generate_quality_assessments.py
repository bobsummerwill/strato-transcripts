...
This script computes lightweight (no-LLM) metrics for:
  - word-count retention (.txt word count vs intermediate baseline)
  - timestamp preservation (.md timestamp counts vs baseline)
...
and optionally a combined:
  outputs/ALL_EPISODES_QUALITY_ASSESSMENT.txt

Design goals:
  - deterministic, fast, no external services
  - matches the newer, metrics-based style used by
    `outputs/episode008-michael-parenti/QUALITY_ASSESSMENT.txt`
"""
    ...
    *,
    ...
    for tr in transcribers:
        ...
        for pr in processors:
            out_txt = out_dir / f"{episode}_{tr}_{pr}.txt"
            out_md = out_dir / f"{episode}_{tr}_{pr}.md"
            if not out_txt.exists() or not out_md.exists():
                continue
            tm: TxtMetrics = compute_txt_metrics(out_txt)
    ...
    lines.append(
        ...
        "  Post-processing should run on the intermediate *.md transcripts (not *.txt)\n"
    ...
    lines.append("  • Use post-processed .txt outputs for corrected wording / readability.")
    lines.append("")
...
def _write_all_episodes_report(summaries: list[dict[str, object]], generated_date: str) -> None:
    out_path = Path("outputs") / "ALL_EPISODES_QUALITY_METRICS.txt"
    lines: list[str] = []
    lines.append("=" * 80)
    ...
    for s in summaries:
        ep = s["episode"]
        lines.append(f"- outputs/{ep}/QUALITY_ASSESSMENT.txt")
    lines.append("")
    lines.append("=" * 80)
...
def main() -> None:
    ...
    ap.add_argument(
        "--write-all-episodes",
        action="store_true",
        help="Also regenerate outputs/ALL_EPISODES_QUALITY_METRICS.txt",
    )
    ap.add_argument(
        "--output-filename",
        default="QUALITY_METRICS.txt",
        help="Episode-level output filename (default: QUALITY_METRICS.txt)",
    )
    ap.add_argument(
        "--force",
        action="store_true",
        help="Overwrite existing outputs/<episode>/QUALITY_ASSESSMENT.txt files",
    )
    args = ap.parse_args()
    ...
    if args.write_all_episodes:
        ...
        print("Wrote outputs/ALL_EPISODES_QUALITY_METRICS.txt")
